...
listsub.size?b.endPullupToRefresh(!0):b.endPullupToRefresh(!1)})},load:function(b){Ajax.post(Ajax.url.topicList,{page:listsub.page,typeId:listsub.topicType},function(a){var f="",d,e,c;for(c in a){e=d="";for(var g in a[c].listImageUrl)d+='<div class="mui-table-view-cell mui-media mui-col-sm-3" id="%{id}"><img src="%{image}" width=60 height=60></div>'.replace("%{id}",a[c].id).replace("%{image}",Constant.hostname+a[c].listImageUrl[g]);0!=a[c].listImageUrl.length&&(e='<div class="mui-table-view mui-grid-view">%{images}</div>'.replace("%{images}",
d));f+=listsub.template.replace("%{id}",a[c].id).replace("%{createTime}",moment((new Date(a[c].createTime)).format("yyyy-MM-dd HH:mm"),"YYYY-MM-DD HH:mm").fromNow()).replace("%{headUrl}",Constant.hostname+a[c].clientHeadUrl).replace("%{nickname}",a[c].clientNickName).replace("%{content}",a[c].content).replace("%{commentCount}",a[c].commentCount).replace("%{listImage}",e)}jq("#topicList").append(f);listsub.addItemListener();void 0!=b&&b(a)})},addItemListener:function(){mui(".mui-table-view").off("tap",
".mui-table-view-cell");mui(".mui-table-view").on("tap",".mui-table-view-cell",function(){var b=this.getAttribute("id");mui.openWindow({id:"detailmain.html",url:"detailmain.html",extras:{_id:b}})})}};
